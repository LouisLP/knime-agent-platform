 ...
 * shared contract, and every failure — network down, non-2xx, malformed body —
 * surfaces as one `ApiError` so callers have a single thing to catch.
 */

const BASE_URL = (import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:3000').replace(/\/$/, '')

/**
 * A transport-level failure: the request never landed. Errors the backend *did*
 * record come back as `error` conversation items instead.
 */
